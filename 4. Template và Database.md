...
for server in serverList:
	print server
```
Cũng tương tự như thế, Jinja2 cung cấp cho chúng ta một vòng lặp for để có thể chạy qua tất cả các giá trị trong một list và in ra. Cách dùng sẽ y hệt như dùng một vòng lặp for thông thường, tất nhiên là trông sẽ hơi loằng ngoằng một chút thôi. File template sẽ có dạng như sau:
```
{% for server in serverList %}
	<li> {{ server }} </li>
{% endfor %}
